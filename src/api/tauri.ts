...
import type { CacheStatus, CostEstimate, DependencyStatus, DownloadProgressEvent, DownloadRequest, ExtractSubtitlesRequest, GeminiSettings, MediaSubtitleAnalysis, ParseVideoRequest, SubtitleProgressEvent, SubtitleProject, SubtitleProjectWorkspace, SubtitleSegment, SubtitleStyle, ToolInstallEvent, VideoInfo, WhisperDownloadEvent, WhisperModelInfo, WhisperRuntimeInfo } from '../types'
...
export function extractSubtitles(request: ExtractSubtitlesRequest) {
  return invoke<string>('extract_subtitles', { request })
}
...
export const listOpenAiCompatibleModels = (request: { apiBase: string; apiKey?: string }) =>
  invoke<string[]>('list_openai_compatible_models', { request })
export const testGlmConnection = () => invoke<string>('test_glm_connection')
...
export const getSubtitleProject = (projectId: string) =>
  invoke<SubtitleProject>('get_subtitle_project', { projectId })
export const saveSubtitleProjectWorkspace = (projectId: string, workspace: SubtitleProjectWorkspace) =>
  invoke<SubtitleProject>('save_subtitle_project_workspace', { request: { projectId, workspace } })
...
export const startGlmTranslation = (request: { projectId: string; targetLanguage?: string }) =>
  invoke<SubtitleProject>('start_glm_translation', { request })
...
export const startGlmPolish = (request: { projectId: string; targetLanguage?: string }) =>
  invoke<SubtitleProject>('start_glm_polish', { request })
